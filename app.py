...
if file:
    try:
        # --- Try to find the first row of actual usage data ---
        ...
        df_raw = pd.read_csv(file, skiprows=start_row)
        
        # Keep only Date and Import (kWh)
        ...
        if not usage_col_candidates:
            ...
            if not date_col_candidates:
                ...
                df = df_raw[[date_col_candidates[0], usage_col]].copy()
                df.rename(columns={date_col_candidates[0]: '15 Min Interval'}, inplace=True)
